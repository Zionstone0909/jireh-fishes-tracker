...
router.post('/', async (req, res) => {
  const { name, email, phone, address, type, notes, balance, totalSpent, createdBy, createdByName } = req.body;
  ...
  try {
    ...
    const result = await pool.request()
      ...
      .input('type', sql.NVarChar, defaultCustomer.type)
      .input('notes', sql.NVarChar, defaultCustomer.notes)
      ...
      .query(`
        INSERT INTO Customers (id, name, email, phone, address, type, notes, totalSpent, balance, status, lastVisit, createdBy, createdByName)
        OUTPUT INSERTED.*
        VALUES (@id, @name, @email, @phone, @address, @type, @notes, @totalSpent, @balance, @status, @lastVisit, @createdBy, @createdByName)
      `);
